...
private val guides = mutableListOf<TvGuide>()
private val movies = mutableListOf<MovieChannel>()
private val playlists = mutableListOf<Playlist>()
private val tvs = mutableListOf<TvChannel>()

val mockChannelGroup = ChannelGroup(
    name = "group1",
    type = ChannelGroup.Type.TV,
    imageUrl = Url( "http://image.jpg" )
)

val mockMovieChannel = MovieChannel(
    id = "movieChannelId",
    name = "movieChannel",
...
val mockLocalData = mockk<LocalData> {
    ...
    fun findList(channel: IChannel ) = when ( channel ) {
        is MovieChannel -> movies
        is TvChannel -> tvs
        else -> throw ChannelNotImplementedException( this@mockk::class, this@mockk::storeChannel, channel::class )
    } as MutableList<IChannel>

    every { channel( any() ) } answers {
        val id = firstArg<String>()
        movies.find { it.id == id } ?: tvs.find { it.id == id }!!
    }

    every { deleteAllChannels() } answers {
        movies.clear()
        tvs.clear()
    }

    every { playlists() } answers { playlists }

    every { storeChannel( any() ) } answers { updateChannel( firstArg() ) }

    every { storePlaylist( any() ) } answers {
        val playlist = firstArg<Playlist>()

        val oldPlaylist = playlists.find { it.path == playlist.path }
        playlists[oldPlaylist] = playlist
    }

    every { tvChannels() } answers { tvs }

    every { updateChannel( any() ) } answers {
        val channel = firstArg<IChannel>()
        val list = findList( channel )

        val oldChannel = list.find { it.id == channel.id }
        list[oldChannel] = channel
    }
